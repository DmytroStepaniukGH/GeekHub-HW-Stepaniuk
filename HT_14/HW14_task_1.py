...
class Atm:
    ...
    @staticmethod
    def get_count_denomination(denomination, combination_denom,
        ...
        while amount_to_pay != 0:
            if sum(available_money.values()) != 0:
                for denomination in denominations:
                    ...
                    if sum(combination_denom.values()) == 0:
                        status = False
                        break
                    else:
                        for key, num in \
    ...
    @staticmethod
    def print_current_exchange_rate():
    ...
    def user_menu(self):
        ...
        while in_action:
            action = input(Fore.BLUE + 'Введіть дію:\n  '
                                       ...
                                       '5. Переглянути поточний курс валют\n  '
                                       '6. Вихід\n  '
                                       'Ваша дія: ')
            print('\n')
            try:
                if int(action) == 1:
                    ...
                    user.print_current_exchange_rate()
                elif int(action) == 6:
                    in_action = False
                else:
